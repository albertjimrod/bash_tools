...
eliminando todas las etiquetas XML y su contenido.
"""
import sys
import re
import argparse
...
def remove_xml_tags(text):
    """Elimina todas las etiquetas XML y su contenido del texto"""
    # Eliminar la declaración XML si existe
    text = re.sub(r'<\?xml.*?\?>', '', text)
    
    # Eliminar etiquetas XML y su contenido
    # Esta expresión regular busca cualquier etiqueta XML y todo su contenido
...
def process_text_for_tags(input_file, output_file=None, patterns=None, model="deepseek-r1:14b"):
    ...
    # Eliminar todas las etiquetas XML y su contenido
    clean_content = remove_xml_tags(original_content)
    
    # Guardar temporalmente el contenido limpio para que Fabric lo analice
    ...
    for pattern in patterns:
        ...
        output = run_fabric_pattern(temp_file, pattern, model)
        if output:
    ...
    # Preparar el contenido final (usando el contenido limpio, no el original con XML)
...
def main():
    ...
    parser.add_argument('--keep-xml', action='store_true', help='Mantener etiquetas XML en el resultado')
