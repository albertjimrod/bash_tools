...
def guardar_en_obsidian(nombre_archivo, contenido):
    """Guarda el contenido en un archivo Markdown para Obsidian."""
    # Sanitizar el nombre del archivo para que sea seguro y compatible con Obsidian
    nombre_archivo_seguro = nombre_archivo.replace(" ", "-").replace(":", "").replace("/", "").replace("\\", "") + ".md"
    try:
        with open(nombre_archivo_seguro, 'w', encoding='utf-8') as f:
            f.write(contenido)
        print(f"\nEl contenido se ha guardado en el archivo: {nombre_archivo_seguro}")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if etiquetas:
        contenido_fabric, titulo_nota = formatear_para_obsidian(ruta_archivo, etiquetas, obtener_contenido_fabric(ruta_archivo))
        if contenido_fabric:
            guardar_en_obsidian(titulo_nota, contenido_fabric)
            print("\nContenido formateado para Obsidian:")
            print(contenido_fabric) # También imprimimos el contenido por si el usuario quiere verlo en la consola
